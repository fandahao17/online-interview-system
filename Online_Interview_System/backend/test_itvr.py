...
class TestInterviewer(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Room.objects.create(roomid=1, time=1, tester='aa@ustc.edu.cn', Interviewee='A@mail.ustc.edu.cn')
    ...
    def test_itvr_set(self):
        c = Client()
        response = c.post('/itvr', {'name':'dd', 'mobile': '12345674567', 'new_email': 'dd@ustc.edu.cn', 'free2': False})
        self.assertEqual(eval(response.content)['success'], True)
        response = c.post('/itvr', {'name':'aa', 'mobile':'17300000000', 'old_email': 'aa@mail.ustc.edu.cn'})
        self.assertEqual(eval(response.content)['success'], True)
    
    def test_itvr_set_time(self):
        c = Client()
        response = c.post('/itvr/setfreetime', {'email': 'bb@ustc.edu.cn', 'free1': False})
        self.assertEqual(eval(response.content)['success'], True)

    def test_itvr_getall(self):
        c = Client()
        response = c.get('/itvr/getall')
        self.assertEqual(len(eval(response.content)), 3)
    
    def test_itvr_delete(self):
        c = Client()
        response = c.get('/itvr/delete', {'name':'cc', 'email':'cc@mail.ustc.edu.cn'})
        self.assertEqual(eval(response.content)['success'], True)
    ...
    def test_itvr_get_itve(self):
        ...
        '''
        c = Client()
        response = c.get('itvr/getitvew/aa@ustc.edu.cn/')
