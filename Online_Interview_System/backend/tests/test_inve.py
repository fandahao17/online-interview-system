...
class TestInterviewee(TestCase):
    ...
    def setUpTestData(cls):
        #创建三条候选人信息
        Interviewee.objects.create(name='A', email='a@mail.ustc.edu.cn', status=0)
        Interviewee.objects.create(name='B', email='b@mail.ustc.edu.cn', status=1)
        Interviewee.objects.create(name='C', email='c@ustc.edu.cn', status=0)
    ...
    def test_itve_getun(self):
        ...
        self.assertEqual(len(json.loads(response.content.decode('utf8'))), 2)
    ...
    def test_itve_delete(self):
        c = Client()
        response = c.delete('/itve/delete/')
        self.assertEqual(json.loads(response.content.decode('utf8'))['success'], True)
        print(Interviewee.objects.all())
